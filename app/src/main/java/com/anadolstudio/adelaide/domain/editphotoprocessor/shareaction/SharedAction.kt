...
interface SharedAction {
    ...
    open class Base(val appPackage: AppPackages?) : SharedAction {
        ...
        protected fun openGooglePlay(namePackage: String, activity: AppCompatActivity) {
            try {
                activity.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=$namePackage")))
    ...
    sealed class SharedItem(@DrawableRes val drawable: Int, appPackage: AppPackages?) : Base(appPackage)
}